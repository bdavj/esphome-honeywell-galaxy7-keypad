...
// Physical keypad addr (panel is 0x11, keypad 0x10/0x20/etc on real bus,
// but our emu logic has always assumed this checksum address)
...
uint8_t HoneywellGalaxy7Keypad::galaxy_checksum_(const std::vector<uint8_t> &data) {
  uint8_t c = 0xAA;
  for (auto b : data) {
    if (static_cast<uint16_t>(c) + b > 0xFF)
      c++;
    c = static_cast<uint8_t>(c + b);
  }
  return c;
}

void HoneywellGalaxy7Keypad::send_frame(const std::vector<uint8_t> &payload, LastCmd cmd) {
  if (this->bus_ == nullptr) {
    ESP_LOGE(TAG, "No GalaxyBus bound, cannot send");
    return;
  }

  ESP_LOGV(TAG, "TX (%d bytes, no cs): %s", (int) payload.size(), bytes_to_hex(payload).c_str());

  if (!this->bus_->enqueue_frame(this, payload)) {
    ESP_LOGW(TAG, "Bus busy, failed to enqueue cmd=%d", (int) cmd);
    return;
  }
...
void HoneywellGalaxy7Keypad::set_display_text_nobl(const std::string &text) {
  ...
  this->parse_display_text_();
  this->screen_dirty_ = true;
  ESP_LOGD(TAG, "Display text (no BL) set to: %s", this->display_text_.c_str());
}
...
std::vector<uint8_t> HoneywellGalaxy7Keypad::build_screen_frame_() {
  ...
  // Toggle 0x80 each screen
  this->screen_seq_flag_ = (this->screen_seq_flag_ == 0x00) ? 0x80 : 0x00;
  modifier |= this->screen_seq_flag_;

  if (this->needs_button_ack_) {
    modifier |= 0x10;              // "screen carries key ACK"
    modifier |= this->ack_toggle_; // 0x00/0x02
    ESP_LOGVV(TAG, "Building screen with button ACK: flags=%02X", modifier);
  ...
  frame.push_back(0x02);  // cursor to second line
  frame.insert(frame.end(), line2.begin(), line2.end());
  frame.push_back(0x07);  // hide cursor
...
void HoneywellGalaxy7Keypad::loop() {
  ...
  if (!this->awaiting_reply_) {
    ...
    if (this->need_reinit_after_f2_) {
      ...
      this->send_frame({this->device_id_, 0x00, 0x0F}, CMD_POLL_00);
      this->last_init_poll_ = now;
      this->ack_toggle_ = 0x02;
      this->screen_seq_flag_ = 0x00;
    ...
    if (cmd_to_send != CMD_NONE) {
      switch (cmd_to_send) {
        ...
        case CMD_SCREEN_07: {
          auto screen = this->build_screen_frame_();
          this->send_frame(screen, CMD_SCREEN_07);
        ...
        case CMD_BEEP_0C:
          ...
          this->send_frame(
              {this->device_id_, 0x0C, this->beep_mode_, this->beep_period_, this->beep_quiet_period_},
              CMD_BEEP_0C);
          this->beep_set_ = true;
          break;
        ...
        case CMD_BACKLIGHT_0D: {
          ...
          this->send_frame({this->device_id_, 0x0D, val}, CMD_BACKLIGHT_0D);
...
std::pair<std::string, bool> HoneywellGalaxy7Keypad::decode_key_and_tamper_(uint8_t code) {
  ...
  switch (key_code) {
    case 0x00: return {"0", tamper};
    case 0x01: return {"1", tamper};
    case 0x02: return {"2", tamper};
    case 0x03: return {"3", tamper};
    case 0x04: return {"4", tamper};
    case 0x05: return {"5", tamper};
    case 0x06: return {"6", tamper};
    case 0x07: return {"7", tamper};
    case 0x08: return {"8", tamper};
    case 0x09: return {"9", tamper};
    case 0x0A: return {"B", tamper};
    case 0x0B: return {"A", tamper};
    case 0x0C: return {"ENT", tamper};
    case 0x0D: return {"ESC", tamper};
    case 0x0E: return {"*", tamper};
    case 0x0F: return {"#", tamper};
    default:   return {"", tamper};
  }
}
...
void HoneywellGalaxy7Keypad::update_tamper_state_(bool new_tamper, const std::string &context) {
  ...
  if (this->tamper_sens_ != nullptr) {
    this->tamper_sens_->publish_state(this->in_tamper_);
  }
}

void HoneywellGalaxy7Keypad::publish_code_(const std::string &code) {
  if (this->code_sens_ == nullptr)
    return;

  this->code_sens_->publish_state(code);

  // clear shortly after so identical codes re-fire state changes
  this->set_timeout("clear_code_sens", 200, [this]() {
    if (this->code_sens_ != nullptr) {
      this->code_sens_->publish_state("");
    }
  });
}

void HoneywellGalaxy7Keypad::sync_beep_switch_state_() {
  if (this->beep_switch_ == nullptr)
    return;
  bool enabled = (this->beep_mode_ == 0x01);
...
void HoneywellGalaxy7Keypad::update_panel_online_state_(bool online) {
  if (online == this->panel_online_)
    return;

  this->panel_online_ = online;
  if (this->panel_online_sens_ != nullptr) {
    this->panel_online_sens_->publish_state(online);
  }

  if (online) {
    ESP_LOGI(TAG, "Panel came online, re-init beep + screen");
    this->beep_set_ = false;
    this->screen_dirty_ = true;
  } else {
    ESP_LOGW(TAG, "Panel offline");
  }
}

// ===== GalaxyBusClient callbacks =====

void HoneywellGalaxy7Keypad::on_bus_frame(const std::vector<uint8_t> &bytes) {
  if (bytes.empty())
    return;

  // For keypad transactions we expect frames from KEYPAD_ADDR
  if (bytes[0] != KEYPAD_ADDR)
    return;

  uint32_t now = millis();
  this->last_panel_rx_ms_ = now;
  this->update_panel_online_state_(true);

  // Only push raw RX debug frames when compiled with VERBOSE logging to avoid spamming DEBUG logs.
#if ESPHOME_LOG_LEVEL >= ESPHOME_LOG_LEVEL_VERBOSE
  if (this->rx_debug_sens_ != nullptr) {
    this->rx_debug_sens_->publish_state(bytes_to_hex(bytes));
  }
#endif

  this->handle_reply_for_cmd_(bytes);
  this->awaiting_reply_ = false;
}

void HoneywellGalaxy7Keypad::on_bus_error(galaxybus::BusError err) {
  ESP_LOGW(TAG, "Bus error for keypad: %d", (int) err);
  this->awaiting_reply_ = false;
}

// ===== reply handler (unchanged logic, just called via on_bus_frame) =====
...
void HoneywellGalaxy7Keypad::handle_reply_for_cmd_(const std::vector<uint8_t> &bytes) {
  ...
  // Screen write: F2 => bad frame, schedule re-init + resend
...
void HoneywellBeepSwitch::write_state(bool state) {
  if (this->parent_ != nullptr) {
    this->parent_->handle_beep_switch_state_(state);
  }
