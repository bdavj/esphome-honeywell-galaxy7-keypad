...
void HoneywellGalaxy7Keypad::bump_backlight_(const char *reason) {
  ...
  ESP_LOGV(TAG, "Backlight bump (%s) until %u ms", reason, this->backlight_off_at_);
}
    ...
    if (cmd_to_send != CMD_NONE) {
      switch (cmd_to_send) {
        ...
        case CMD_SCREEN_07: {
          ...
          screen.insert(screen.end(), this->display_line1_.begin(), this->display_line1_.end());
          ...
          ESP_LOGD(TAG, "Sending screen frame (%d bytes): %s", (int) frame.size(), bytes_to_hex(frame).c_str());
  ...
  if (type == 0xF4 && bytes.size() == 4) {
    ...
    if (this->last_cmd_ == CMD_SCREEN_07) {
      if (code == 0x7F) {
        ...
        ESP_LOGV(TAG, "Screen reply key=%s%s %s", key_name.c_str(), tamper ? " [TAMPER]" : "",
    ...
    // De-dupe identical key within 200 ms
    bool duplicate = (key_name == this->last_key_name_) && (tamper == this->last_key_tamper_) &&
                     (millis() - this->last_key_ts_ <= 200);
    if (!duplicate) {
      ESP_LOGI(TAG, "Key=%s%s %s", key_name.c_str(), tamper ? " [TAMPER]" : "", bytes_to_hex(bytes).c_str());
      this->last_key_name_ = key_name;
      this->last_key_tamper_ = tamper;
      this->last_key_ts_ = millis();
    }

    // ACK back to keypad (0B) for any key-bearing F4 and flip toggle
